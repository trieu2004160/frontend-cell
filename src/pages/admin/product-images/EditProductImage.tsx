import { Form, InputNumber, Select, Spin } from "antd";
import TextArea from "antd/es/input/TextArea";
import { useEffect, useState } from "react";
import ButtonCellphoneS from "../../../components/ButtonCellphoneS";
import UploadImage from "../../../components/admin/UploadImage";
import { useNavigate, useParams } from "react-router-dom";
import { useMessage } from "../../../hooks/useMessage";
import type { ProductImagesProp } from "../../../types/api/ProductImageResponse";
import { productApi } from "../../../utils/api/product.api";
import type {
  ProductResponse,
  ProductSelect,
} from "../../../types/api/ProductResponse";
import { productImgaesApi } from "../../../utils/api/product_images.api";

const EditProductImage = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const [form] = Form.useForm();
  const [imageApi, setImageApi] = useState<string | undefined>("");
  const [loading, setLoading] = useState(true);
  const { showSuccess, showError, contextHolder } = useMessage();
  const [allNameProduct, setAllNameProduct] = useState<
    ProductResponse<ProductSelect>["data"]
  >([]);

  const handleFinish = async (value: ProductImagesProp) => {
    try {
      const result = await productImgaesApi.update(Number(id), {
        ...value,
        image_url: imageApi,
      });
      showSuccess(result.message || "Image updated successfully");
      setTimeout(() => {
        navigate(-1);
      }, 1500);
    } catch (error) {
      showError((error as any).message || "An error occurred");
    }
  };

  const handleImageApi = (image_url: string | undefined) => {
    setImageApi(image_url);
  };

  const getAllNameProduct = async () => {
    try {
      const result = await productApi.getAllName();
      setAllNameProduct(result.data);
    } catch (error) {
      console.log(error);
    }
  };

  const fetchProductImageDetail = async () => {
    try {
      setLoading(true);
      const result = await productImgaesApi.getById(Number(id));
      const data = Array.isArray(result.data) ? result.data[0] : result.data;

      form.setFieldsValue({
        product_id: data.product_id,
        image_type: data.image_type,
        sort_order: data.sort_order,
        alt_text: data.alt_text,
      });
      setImageApi(data.image_url);
    } catch (error) {
      showError((error as any).message || "Failed to fetch image details");
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    getAllNameProduct();
    if (id) {
      fetchProductImageDetail();
    }
  }, [id]);

  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <Spin size="large" />
      </div>
    );
  }

  return (
    <>
      {contextHolder}
      <div className="bg-[#f5f5f5] rounded-lg p-4 mt-4 mb-[2rem]">
        <h1 className="text-2xl font-semibold mb-4">Edit Product Image</h1>
        <Form
          form={form}
          labelCol={{ span: 24 }}
          wrapperCol={{ span: 24 }}
          onFinish={handleFinish}
        >
          <div className="flex md:flex-row flex-col md:items-center gap-x-4">
            <Form.Item
              label="Product"
              name="product_id"
              className="md:w-[20rem] w-full"
              rules={[{ required: true, message: "Please select a product!" }]}
            >
              <Select
                showSearch
                options={allNameProduct}
                placeholder="Select product"
                className="h-[2.5rem]"
              />
            </Form.Item>
            <Form.Item
              label="Image Type"
              name="image_type"
              rules={[{ required: true, message: "Please select image type!" }]}
            >
              <Select
                options={[
                  {
                    label: "Main",
                    value: "main",
                  },
                  {
                    label: "Gallery",
                    value: "gallery",
                  },
                  {
                    label: "Variant",
                    value: "variant",
                  },
                ]}
                placeholder="Select image type"
                className="h-[2.5rem]"
              />
            </Form.Item>
            <Form.Item label="Sort order" name="sort_order">
              <InputNumber min={0} defaultValue={0} className="py-1 w-[5rem]" />
            </Form.Item>
          </div>

          <Form.Item label="Image" name="image_url">
            <UploadImage setImageApi={handleImageApi} initialImage={imageApi} />
          </Form.Item>
          <Form.Item>
            <div className="flex items-center justify-end gap-x-4">
              <ButtonCellphoneS
                children="Cancel"
                className="w-[6rem] bg-white"
                defaultHoverBg="none"
                onClick={() => navigate(-1)}
              />
              <ButtonCellphoneS
                htmlType="submit"
                children="Update"
                className="w-[6rem] text-white"
              />
            </div>
          </Form.Item>
        </Form>
      </div>
    </>
  );
};

export default EditProductImage;
